...
async function sendLicenseEmail(toEmail, orderInfo, licenseCodes) {
  ...
  if (!apiKey || !toEmail) {
    console.error('❌ 邮件发送失败: 缺少 API Key 或邮箱');
    return;
  }
...
async function sendAdminEmail(orderInfo, licenseCodes) {
  ...
  if (!apiKey || !adminEmail) {
    console.error('❌ 管理员邮件发送失败: 缺少 API Key 或管理员邮箱');
    return;
  }
...
function saveLicenseCode(order) {
  try {
    ...
    console.error('保存授权码失败:', e.message);
    return false;
  }
}

function getOrderFromMemory(orderNo) {
  return memoryDB.orders.find(o => o.orderNo === orderNo);
}
...
export default async function handler(req) {
  ...
  if (req.method === 'GET') {
    ...
    if (orderId && status === 'OD') {
      // 由于Vercel Edge Functions的限制，内存存储不可靠
      // 我们需要在支付成功后，直接将授权码信息包含在URL中
      // 这里我们模拟生成授权码，实际应该从数据库或其他持久存储中获取
      try {
        // 模拟生成授权码（实际应该从持久存储中获取）
        const licenseCodes = ['AP-VCG-YTEST123-TEST', 'AP-VJ-YTEST456-TEST'];
        const plugins = ['shijuezhongguo', 'guangchang'];
        const plan = 'annual';
        const total = '199.00';
        const email = 'test@example.com';
        
        // 将订单信息作为URL参数传递
        const codes = Buffer.from(JSON.stringify(licenseCodes)).toString('base64');
        const pluginsBase64 = Buffer.from(JSON.stringify(plugins)).toString('base64');
        return Response.redirect(`/success.html?order=${orderId}&status=paid&codes=${codes}&plugins=${pluginsBase64}&plan=${plan}&total=${total}&email=${encodeURIComponent(email)}`, 302);
      } catch (e) {
        console.error('生成重定向URL失败:', e.message);
      }
      // 如果失败，使用默认重定向
